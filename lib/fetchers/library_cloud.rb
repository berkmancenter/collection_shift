...
class LibraryCloud
    ...
    LIMIT = 250
    MAX_RETURNED_RECORDS = 7500

    def initialize
        @item_api = LibraryCloudItems.new
    end

    def pages_in_range(library_code, start_call_num, end_call_num)
        ...
        loop do
            ...
            results['docs'].each do |doc|
                ...
                if doc['format'] == 'Serial'
                    count = @item_api.count(library_code, doc['id_inst'])
                    if count
    ...
    def pages_to_int(pages)
        ...
        segments.delete_if{|segment| segment.match(all_words) || segment.match(roman_numeral) || segment.match(unwanted_chars) || segment.match(number_in_brackets)}
